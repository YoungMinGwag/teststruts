...
<body bgcolor="<%=bodyback_c%>">
	...
	<table width=500 border=1 cellspacing=0 cellpadding=0
		...
		<tr height=30>
			<td colspan=4 bgcolor=<%=value_c %> align=right><input
				...
				onclick="document.location.href='writeForm.jsp?num=<%=num%>&ref=<%=ref%>&re_setp=<%=re_setp%>&re_level=<%=re_level%>'">&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    //전체 패이지의 수를 연산
        ...
        if (startPage > 5) {    %>
				<a
					href="content1.jsp?num=<%=num %>&pageNum=<%=pageNum %>&cPageNum=<%= startPage - 5 %>">[이전]</a>
				<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
				<a
					href="content1.jsp?num=<%=num %>&pageNum=<%=pageNum %>&cPageNum=<%= i %>">[<%= i %>]
				</a>
				<%
        }
        if (endPage < pageCount) {  %>
				<a
					href="content1.jsp?num=<%=num %>&pageNum=<%=pageNum %>&cPageNum=<%= startPage + 5 %>">[다음]</a>
				<%
        }
    }
%>
