...
public class Sequencer {
	...
	public synchronized static int nextId(Connection conn, String tableName) throws SQLException {
		...
		try {
			...
			pstmtSelect = conn.prepareStatement("select MESSAGE_ID from ID_SEQUENCES_YM where TABLE_NAME = ?");
			...
			if (rsSelect.next()) {
				...
				pstmtUpdate = conn.prepareStatement(
						"update ID_SEQUENCES_YM set MESSAGE_ID = ? "
				...
				pstmtUpdate = conn.prepareStatement(
						...
						"insert into ID_SEQUENCES_YM values (?, ?)");
